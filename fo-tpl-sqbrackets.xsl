    ...
    <xsl:template match="text()" mode="sqbrackets">
        ...
        <xsl:variable name="ignorableThingsBetweenAdjacentBrackets">
            <regexselectors>
                ...
                <!-- editorial comma -->
                <selector>,</selector>
                <!-- add another one below if something else needs to be ignored when removing adjacent ][ -->
            </regexselectors>
        </xsl:variable>
        ...
        <xsl:variable name="openingBrackets" select="$openingadjacentbrackettypes/closingbrackets/bracket ! replace(., '\\', '')"/>
        <xsl:variable name="closingBrackets" select="$closingadjacentbrackettypes/closingbrackets/bracket ! replace(., '\\', '')"/>
        ...
        <xsl:choose>
            <xsl:when test="$firstletter = $openingBrackets or $lastletter = $closingBrackets">
                ...
                <xsl:choose>
                    <xsl:when test="$firstletter = $openingBrackets and $lastletter = $closingBrackets and $plastletter = $closingBrackets and $afirstletter = $openingBrackets and generate-id($afterprevbr) != generate-id(.) and generate-id($beforenextbr) != generate-id(.)">
                        <xsl:value-of select="substring($current, 2, string-length($current)-2)"/>
                    </xsl:when>
                    <xsl:when test="$plastletter = $closingBrackets and $firstletter = $openingBrackets and generate-id($afterprevbr) != generate-id(.)">
                        <xsl:value-of select="substring($current, 2)"/>
                    </xsl:when>
                    <xsl:when test="$afirstletter = $openingBrackets and $lastletter = $closingBrackets and generate-id($beforenextbr) != generate-id(.)">
