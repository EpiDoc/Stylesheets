    ...
    <xsl:template match="text()" mode="sqbrackets">
        ...
        <xsl:choose>
            <xsl:when test="$firstletter = $openingBrackets or $lastletter = $closingBrackets">
                ...
                <xsl:choose>
                    <xsl:when test="$plastletter = $closingBrackets and $afirstletter = $openingBrackets and $afterprevbr != . and $beforenextbr != .">
                        <xsl:value-of select="substring($current, 2, string-length($current)-2)"/>                                                
                    </xsl:when>
                    <xsl:when test="$plastletter = $closingBrackets and $afterprevbr != .">
                        <xsl:value-of select="substring($current, 2)"/>
                    </xsl:when>
                    <xsl:when test="$afirstletter = $openingBrackets and $beforenextbr != .">
