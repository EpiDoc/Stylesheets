...
<xsl:stylesheet xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:t="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="t" version="2.0">
  <!-- Contains named templates for medcyprus file structure (aka "metadata" aka "supporting data") -->  
   ...
   <xsl:template name="medcyprus-body-structure">
     ...
     <p>
       <b>License: </b> <a target="_blank" href="{//t:licence/@target}">
                <xsl:value-of select="//t:licence"/>
            </a>
       <br/>
            <b>Authority: </b> <xsl:apply-templates select="//t:publicationStmt/t:authority"/>
       <br/>
            <b>Identification number: </b> <xsl:value-of select="//t:publicationStmt/t:idno[@type='filename']"/>
       <br/>
            <b>Editor(s): </b> <xsl:for-each select="//t:titleStmt/t:author|//t:titleStmt/t:editor">
         <xsl:value-of select="."/>
     ...
     <p>
       ...
       <br/>
            <b>Articulation of inscription in relation to the murals: </b>
     ...
     <div class="section-container tabs" data-section="tabs">
       <section><!-- #1 interpretative (verse-lines off) -->
         <p class="title" data-section-title="data-section-title">
                    <a href="#">
                        <i18n:text i18n:key="epidoc-xslt-medcyprus-edition">Interpretative</i18n:text>
                    </a>
                </p>
       ...
       <xsl:if test="//t:div[@type='edition'][descendant::t:lg or descendant::t:l]">
         <section><!-- #2 verse (optional) -->
           <p class="title" data-section-title="data-section-title">
                        <a href="#">
                            <i18n:text i18n:key="epidoc-xslt-medcyprus-edition">Verse</i18n:text>
                        </a>
                    </p>
           <div class="content" id="edition" data-section-content="data-section-content">
             <!-- Edited text output -->
       ...
       <section><!-- #3 diplomatic -->
         <p class="title" data-section-title="data-section-title">
                    <a href="#">
                        <i18n:text i18n:key="epidoc-xslt-medcyprus-diplomatic">Diplomatic</i18n:text>
                    </a>
                </p>
       ...
       <xsl:if test="//t:facsimile//t:graphic[@rend='primary'][1]">
         <section>
           <p class="title" data-section-title="data-section-title">
             <a href="#">
                            <i18n:text i18n:key="epidoc-xslt-medcyprus-image">Image</i18n:text>
                        </a>
                    </p>
  ...
  <xsl:template match="t:dimensions" mode="medcyprus-dimensions">
      <xsl:if test="//text()">
        <xsl:if test="t:height/text()">
          <xsl:if test="normalize-space(t:height/text()) != 'n/a'">
                    <xsl:text>h: </xsl:text>
                </xsl:if> 
  ...
  <xsl:template match="t:placeName|t:origPlace|t:repository" mode="medcyprus-location">
      <xsl:choose>
        <xsl:when test="starts-with(@ref, 'locations.xml') or starts-with(@ref, '#')">
                <a href="../indices/epidoc/locations.html#{substring-after(@ref, '#')}">
                    <xsl:apply-templates/>
                </a>
            </xsl:when>
