...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" version="2.0">
    ...
    <xsl:template match="t:ab">
      ...
      <xsl:if test="following::t:lb[1][@break='no' or @type='inWord'] and not($parm-edition-type='diplomatic')           and not(starts-with($parm-leiden-style, 'edh') or $parm-leiden-style='eagletxt')">
          <xsl:text>-</xsl:text>
      </xsl:if>
  </xsl:template>
